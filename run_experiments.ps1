...
# large : 10000 items, 100 gen, pop 100 -> evaluacion real del paralelismo
# ---------------------------------------------------------------------------
$AllInstances = @(
    @{ Label="small";  Name="data/small";  Gen=500; Pop=200 },
    @{ Label="medium"; Name="data/medium"; Gen=300; Pop=200 },
    @{ Label="large";  Name="data/large";  Gen=100; Pop=100 }
)
...
foreach ($inst in $Instances) {
    Write-Host "`n--- Instancia: $($inst.Label) (gen=$($inst.Gen), pop=$($inst.Pop)) ---" -ForegroundColor Yellow
